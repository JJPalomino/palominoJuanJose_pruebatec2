...
public class CiudadanoJpaController implements Serializable {
    ...
    public List<Ciudadano> findTramiteEntities() {
        EntityManager em = getEntityManager();
        try {
            return em.createQuery("SELECT c FROM Ciudadano c WHERE c.estado = true", Ciudadano.class)
                 .getResultList();
        } finally {
            em.close();
        }
    ...
    public Ciudadano findByCURP(String ciudadanoCURP) {
        EntityManager em = getEntityManager();
        try {
            String consulta = "SELECT c FROM Ciudadano c WHERE c.CURP = :CURP";
            Query query = em.createQuery(consulta);
            query.setParameter("CURP", ciudadanoCURP);
